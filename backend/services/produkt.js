...
serviceRouter.post('/produkt', function(request, response) {
    ...
    if (!helper.isNumeric(request.body.preis)) 
        errorMsgs.push('preis muss eine Zahl sein');
    if (helper.isUndefined(request.body.prozente)) 
        request.body.prozente = 0.0;
    if (!helper.isNumeric(request.body.prozente)) 
        errorMsgs.push('prozente muss eine Zahl sein');
    if (request.body.prozente < 0) 
        errorMsgs.push('prozente muss größer 0 sein');
    ...
    const produktDao = new ProduktDao(request.app.locals.dbConnection);
    try {
        var obj = produktDao.create(request.body.kategorie.id, request.body.bezeichnung, request.body.beschreibung, request.body.mehrwertsteuer.id, request.body.details, request.body.preis, request.body.prozente, request.body.datenblatt, request.body.bilder);
...
serviceRouter.put('/produkt', function(request, response) {
    ...
    const produktDao = new ProduktDao(request.app.locals.dbConnection);
    try {
        var obj = produktDao.update(request.body.id, request.body.kategorie.id, request.body.bezeichnung, request.body.beschreibung, request.body.mehrwertsteuer.id, request.body.details, request.body.preis, request.body.prozente, request.body.datenblatt, request.body.bilder);
